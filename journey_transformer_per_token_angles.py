...
class Attention(nn.Module):
  ...
  def forward(self, x, a):
    ...
    angle_cumsum = torch.cumsum(angle, dim=1)
    angle_cumsum = torch.flip(angle_cumsum, dims=(1,))
    cos_angle = torch.cos(angle_cumsum)
    sin_angle = torch.sin(angle_cumsum)
...
class BigramNeuralNetwork(nn.Module):
  ...
  def forward(self, idx, targets=None):
    # idx = idx[:,-block_size:]
    B,T = idx.shape
    x = self.expander(self.token_embedding_table(idx)) # (B,T,C_e)
    a = self.angle_embedding_table(idx)
